...
This project strictly separates the **code** from the **data**.
...
* **DATA (Hugging Face):**
    ...
    * **Contents:** The `.jsonl` data and all scraped `/images` or `/audio`.
...
2.  **Install dependencies:** (You must have Python 3 installed)
    ```bash
...
3.  Rewrite the `get_all_category_pages()` and `scrape_gallery_page()` (or `process_post_json()`) functions to match the new site's HTML structure or API.
